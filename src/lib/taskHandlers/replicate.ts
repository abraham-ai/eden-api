...
const makeWebhookUrl = () => {
  ...
  return `${process.env.WEBHOOK_URL}/model_update_replicate?secret=${webhookSecret}`;
}
...
const submitTask = async (server: FastifyInstance, generatorId: string, config: any) => {
  ...
  const webhookUrl = makeWebhookUrl();

  // const model = await replicate.getModel(generatorId);
  let model;
  try {
  ...
  const task = await replicate.startPrediction(modelId, {text: "test"}, webhookUrl, ['start', 'output', 'completed']);
  return task.id
}
